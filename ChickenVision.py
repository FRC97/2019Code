...
lower_green = np.array([50, 142, 100])
...
def findTape(contours, image, centerX, centerY):
    ...
    if len(contours) >= 2:
        ...
        for i in range(len(biggestCnts) - 1):
            ...
            # Find total tilt b/t the two contours
            totalTilt = (cy2 - cy1)/(cx2 - cx1)
            ...
            if (np.sign(tilt1) != np.sign(tilt2)):
                ...
                if not targets:
                    targets.append([centerOfTarget, yawToTarget, totalTilt])
    ...
    if (len(targets) > 0):
        ...
        networkTable.putNumber("tapeTilt", finalTarget[2])
